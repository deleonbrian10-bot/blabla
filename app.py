...
def _maybe_capture_table(data):
    ...
    try:
        # Gate to Stats page only to keep the picker clean
...
if not getattr(st.dataframe, "__ai_capture__", False):
    ...
    def _dataframe_capture(data=None, *args, **kwargs):
        _maybe_capture_table(data)
        return _ORIG_ST_DATAFRAME(data, *args, **kwargs)
    _dataframe_capture.__ai_capture__ = True
    st.dataframe = _dataframe_capture

if not getattr(st.table, "__ai_capture__", False):
    _ORIG_ST_TABLE = st.table
    def _table_capture(data=None, *args, **kwargs):
        _maybe_capture_table(data)
